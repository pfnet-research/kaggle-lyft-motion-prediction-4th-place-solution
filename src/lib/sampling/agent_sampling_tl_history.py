...
from l5kit.kinematic import Perturbation, perturbation
...
def create_generate_agent_sample_tl_history_partial(cfg, rasterizer):
    ...
    sample_function = partial(
        generate_agent_sample_tl_history,
...
to train models that can recover from slight divergence from training set data
    ...
    all_tl_slice = get_tl_faces_slice_from_frames(all_history_frames[-1], all_history_frames[0])  # -1 is the farthest
    # sync interval with the traffic light faces array
    all_history_frames["traffic_light_faces_index_interval"] -= all_tl_slice.start
    all_history_tl_faces = filter_tl_faces_by_frames(all_history_frames, tl_faces[all_tl_slice].copy())
    ...
    input_im = (
        None
        if not rasterizer
        else rasterizer.rasterize(history_frames, history_agents, all_history_tl_faces, selected_agent)
    )
