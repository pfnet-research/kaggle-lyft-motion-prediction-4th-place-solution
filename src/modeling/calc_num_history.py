...
def calc_num_history(data_loader) -> np.ndarray:
    n_history_availabilities_list = []

    with torch.no_grad():
        dataiter = tqdm(data_loader)
        for data in dataiter:
            n_his_avail = np.sum(data["history_availabilities"].numpy(), axis=1)
            n_history_availabilities_list.append(n_his_avail)
    n_his_availabilities = np.concatenate(n_history_availabilities_list)
    return n_his_availabilities
...
if __name__ == '__main__':
    ...
    # --- Calc valid data ---
    n_his_avail_valid = calc_num_history(valid_loader)
    print("n_his_avail_valid", n_his_avail_valid.shape, n_his_avail_valid)
    ...
    npz_path = processed_dir / f"n_history{debug_str}.npz"
    np.savez_compressed(
        npz_path,
        n_his_avail_valid=n_his_avail_valid,
        n_his_avail_test=n_his_avail_test,
    )
